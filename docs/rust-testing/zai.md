...
## 1. Introduction to Rust Testing Ecosystem
...
Unit tests can access **private functions** and code within the same module, which allows for thorough testing of internal implementation details【turn0search0】. This is a deliberate design choice in Rust, recognizing that testing private functions can sometimes be valuable for ensuring correctness.

### 2.2 Integration Tests

Integration tests are **external to your library** and use your code in the same way any other external code would, using only the public API【turn0search1】. Their purpose is to test whether multiple parts of your library work together correctly. To create integration tests, you create a `tests` directory at the top level of your project directory, next to `src`【turn0search0】.
...
Each Rust source file in the `tests` directory is compiled as a separate crate, which means they can share code through modules but don't share state between files【turn0search1】. You can create a `common` module in `tests/common/mod.rs` to share helper functions across integration tests:
...
Documentation tests are code examples in documentation comments that serve as both documentation and tests【turn0search5】. They're excellent for ensuring that documentation stays up-to-date with the code implementation. Rust's documentation tool, `rustdoc`, automatically extracts and runs these examples as tests.
...
You can also hide parts of examples that are needed for setup but not relevant to the documentation using lines starting with `#`【turn0search5】:
...
proptest! {
    ...
    fn test_add_two_commutes(a: i32, b: i32) {
        // Property: Adding two numbers is commutative
        prop_assert_eq!(a + b, b + a);
    }
    
    #[test]
    fn test_add_two_preserves_order(a: i32, b: i32) {
        // Property: Adding two then subtracting one preserves the other
        prop_assert_eq!((a + b) - b, a);
    }
}
```
...
## 3. Test Runners in Rust

### 3.1 Default Cargo Test Runner
...
### 3.2 Nextest: Next-Generation Test Runner

**Nextest** is a next-generation test runner for Rust that offers significant performance improvements and additional features over the default Cargo test runner【turn0search7】. It's designed for running Rust tests at scale and addresses many real-world problems with running tests in large projects.

Key features of Nextest include:

- **Up to 3x faster** execution than `cargo test` through smarter test execution- **Clean, beautiful user interface** that clearly shows which tests passed and failed- **Powerful test selection** using a sophisticated expression language to filter tests- **Identify misbehaving tests** with slow test detection and termination- **Customize settings by test** with automatic retries, heavy test marking, and serial execution- **Designed for CI** with test archiving, partitioning, and JUnit XML export
```bash
# Install nextest
...
## 4. Testing Best Practices in Rust
...
### 4.2 Test Naming and Conventions
...
// Step 2: Implement minimal code (in this case, Vec already does this)
...
### 4.5 Continuous Integration with Testing
...
impl StateMachine for SystemState {
    type Transition = Transition;
    type ExtState = Self;
    
    fn init_state() -> BoxedStrategy<Self::ExtState> {
        Just(SystemState { map: HashMap::new() }).boxed()
    }
    
    fn transitions(state: &Self::ExtState) -> BoxedStrategy<Self::Transition> {
        let insert = ("key", 0..1000i32).prop_map(|(key, value)| Transition::Insert { 
            key: format!("{}-{}", key, fastrand::u64(..)), 
            value 
        });
        
        let remove = state.map.keys().cloned().prop_map(Transition::Remove);
        let get = state.map.keys().cloned().prop_map(Transition::Get);
        
        prop_oneof![insert, remove, get].boxed()
    }
    
    fn apply(mut state: Self::ExtState, transition: &Self::Transition) -> Self::ExtState {
        match transition {
    ...
    fn preconditions(state: &Self::ExtState, transition: &Self::Transition) -> bool {
        match transition {
            Transition::Remove { key } | Transition::Get { key } => {
                state.map.contains_key(key)
            }
            Transition::Insert { .. } => true,
        }
    }
}
...
fn test_state_machine() {
    proptest!(|(state_machine in StateMachine::new(SystemState::init_state))| {
        state_machine.prop_run(|ref mut state_machine| {
            // Check invariants after each transition
            if let Transition::Insert { key, value } = state_machine.current_transition() {
                prop_assert_eq!(state_machine.ext_state().map.get(key), Some(value));
            }
            Ok(())
        })?;
    });
...
Testing in Rust is a rich ecosystem with multiple approaches and tools. Here are my recommendations for effective testing in Rust:
