...
///
/// This function:
/// 1. Loads the resource (from cache for remote, or filesystem for local)
/// 2. Applies line range filtering if specified
/// 3. Parses the transcluded content as a DarkMatter document
/// 4. Recursively resolves nested transclusions
...
pub fn resolve_transclusion<'a>(
    ...
    match node {
        DarkMatterNode::File { resource, range } => {
            ...
            let content = load_resource(resource, cache, base_path).await?;

            // 2. Apply line range if specified
            let content = apply_line_range(&content, range)?;

            // 3. Parse the transcluded content
            let doc = parse_document(&content, resource.clone())
                .map_err(|e| RenderError::ParseError(e.to_string()))?;

            // 4. Recursively resolve transclusions in the transcluded content
